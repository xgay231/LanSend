...
HttpServer::HttpServer(net::io_context& io_context, ssl::context& ssl_context)
    ...
    , running_(false) {
    Logger::get_instance().info("HttpServer created.");
}
...
HttpServer::~HttpServer() {
    ...
    Logger::get_instance().info("HttpServer destroyed.");
}
...
void HttpServer::start(uint16_t port) {
    if (running_) {
        Logger::get_instance().warn("Server is already running.");
        return;
    }
    ...
    try {
        ...
        running_ = true;
        Logger::get_instance().info(std::format("HTTPS Server started on port {}", port));

        net::co_spawn(io_context_, accept_connections(), net::detached);

    } catch (const std::exception& e) {
        Logger::get_instance().error(
            std::format("Failed to start server on port {}: {}", port, e.what()));
        running_ = false;
...
void HttpServer::stop() {
    ...
    Logger::get_instance().info("HTTPS Server stopped.");
}
    ...
    while (running_) {
        try {
            ...
            if (e.code() == net::error::operation_aborted) {
                Logger::get_instance().info("Accept operation cancelled.");
                break;
            } else {
                Logger::get_instance().error(
                    std::format("Error accepting connection: {}", e.what()));
            }
        } catch (const std::exception& e) {
            Logger::get_instance().error(
                std::format("Unexpected error during accept: {}", e.what()));
        }
    }
    Logger::get_instance().info("Stopped accepting connections.");
}
...
boost::asio::awaitable<void> HttpServer::handle_connection(ssl::stream<tcp::socket> stream) {
    ...
    try {
        ...
        Logger::get_instance().info("SSL handshake successful.");
        ...
        while (stream.next_layer().is_open()) {
            ...
            if (result.index() == 0) {
                if (!timer_ec) {
                    Logger::get_instance().info(
                        std::format("Keep-alive timeout after {}s. Closing connection.",
                    ...
                    Logger::get_instance().error(
                        std::format("Keep-alive timer error: {}. Closing connection.",
                                    timer_ec.message()));
                    break;
                }
            }
        ...
        if (e.code() == net::error::eof || e.code() == ssl::error::stream_truncated
            ...
            Logger::get_instance().info(std::format("Connection closed by peer: {}", e.what()));
        } else if (e.code() != net::error::operation_aborted) {
            Logger::get_instance().error(std::format("Connection error: {}", e.what()));
        }
    } catch (const std::exception& e) {
        Logger::get_instance().error(
            std::format("Unexpected error handling connection: {}", e.what()));
    }
    ...
    try {
        ...
        if (e.code() != net::error::eof && e.code() != ssl::error::stream_truncated
            ...
            Logger::get_instance().warn(std::format("Error during SSL shutdown: {}", e.what()));
        }
    } catch (const std::exception& e) {
        Logger::get_instance().warn(
            std::format("Unexpected error during SSL shutdown: {}", e.what()));
    }
    Logger::get_instance().info("Connection handling finished.");
}
...
boost::asio::awaitable<AnyResponse> HttpServer::handle_request(HttpRequest&& req) {
    ...
    if (it == routes_.end()) {
        Logger::get_instance().warn(std::format("Route not found: {}", path));
    ...
    try {
        ...
        Logger::get_instance().error(
            std::format("Error executing handler for {}: {}", path, e.what()));
